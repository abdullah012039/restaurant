...
export default function MenuPage() {
  ...
  // عند تحميل الصفحة: اجلب قيمة البحث وcategory من URL وضعها في state
  useEffect(() => {
    ...
    if (urlCategory !== state.selectedCategory) {
      dispatch({ type: "SET_SELECTED_CATEGORY", payload: urlCategory });
    }
    // eslint-disable-next-line
  }, [searchParams, dispatch]);

  // عند تغيير البحث أو التصنيف من input: حدث الـ URL فقط إذا تغيرت القيمة
  useEffect(() => {
    ...
    if (state.searchQuery !== currentSearch) {
  ...
  const filteredAndSortedItems = useMemo(() => {
    ...
    if (state.searchQuery) {
      items = items.filter(
        (item) =>
          ...
          item.ingredients?.some((ingredient: string) => ingredient.toLowerCase().includes(state.searchQuery.toLowerCase())),
      );
    }
    ...
    if (selectedAllergens.length > 0) {
      items = items.filter((item) => !item.allergens?.some((allergen: string) => selectedAllergens.includes(allergen)));
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
      <div className="container mx-auto px-4">
        ...
        {/* Search and Filters */}
        <div className="bg-gray-400 dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-8 " style={{ backdropFilter: "blur(10px) contrast(80%)" }}>
          <div className="flex flex-col lg:flex-row gap-4 items-center">
            ...
            >
              ...
              <SelectContent className="max-h-60 bg-gray-400 dark:bg-gray-800">
